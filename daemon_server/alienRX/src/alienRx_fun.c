...
void isr_signal(int signal)
{
	...
	if ( signal == SIGUSR1 )
	{
		syslog(LOG_DEBUG,"Señal SIGUSR1 recibida\n");
		/**
		 * Esperando al proceso hijo para finalizar
		 * */
		pid = wait(&status);
		status>>=8;
		syslog(LOG_DEBUG,"Proceso %d terminado con resultado %d\n",pid,status); 
	}
}
...
int send_data(char *data)
{
	...
	bytes_writed = write( cliente_sockfd, data, len_data ); 
	if( bytes_writed < 0 )
	{
		syslog(LOG_ERR,"Ocurrio un problema al ENVIAR el mensaje\n");
		exit(1);
	}
	return bytes_writed;
}
