	...
	while(1)
	{
		...
		if( !pid ) /* proceso hijo*/
		{
			...
			* */
			 ...
			 * la ruta y nombre de la imagen es "img_proc.bmp"
			 * */
			 /**
			...
			bytes_readed = fread(&buffer, sizeof(unsigned char), TAM_BUFFER, file);
			while ( bytes_readed )
			{
